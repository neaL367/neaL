...
export async function orchestrateRetrieval(
  ...
  // 2. Dispatch local lanes concurrently
  const [kgHit, bm25Results, semanticHits] = await Promise.all([
    ...
    semanticIndex.search(reformulated.cleaned, limit),
  ]);
  ...
  // Lane 3: BM25F Lexical (Weight: 1.0)
  if (bm25Results.length > 0) {
    const bm25Hits: RetrievalHit[] = bm25Results.slice(0, limit).map(match => ({
      id: match.section.id,
