...
- WALLS: the app pins `next dev` and `next build` to webpack, so instant verification uses production `next start`; the post manifest is regenerated by `bun run build`
