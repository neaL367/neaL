...
export function PhotoGrid({ posts, columns = 3, initialSortBy = "newest" }: PhotoGridProps) {
  ...
  return (
    <div>
      <div className="flex justify-start mt-6 mb-4">
        <Button variant="outline" size="sm" onClick={() => setShowSortControls(!showSortControls)}>
          {showSortControls ? "Hide Filters" : "Sort & Filter"}
        </Button>
      </div>

      {showSortControls && (
        <Suspense fallback={<div className="h-12 w-full bg-muted/20 animate-pulse rounded-md mb-4"></div>}>
